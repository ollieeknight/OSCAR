...
#            echo -e "\033[0;33mWriting ${modality} for ${library}\033[0m"
        ...
        if [[ -f ${library_output} ]]; then
            ...
            echo -e "\033[0;31mError when processing ${library_output}.\033[0m Please ensure that in the metadata file, GEX libraries for all samples are first, before ADT/HTO/VDJ-T/CRISPR"
            exit 1
        fi
    fi
    write_fastq_files
