...
if [ ! -f "oscar-counting_latest.sif" ]; then
	echo "Pulling OSCAR counting image, this might take some time..."
	echo ""
	apptainer pull --arch amd64 library://romagnanilab/default/oscar-counting:latest
fi

cd test_run
echo "Running cellranger mkfastq"
echo ""
apptainer run -B /fast ../oscar-counting_latest.sif cellranger mkfastq --id test_run_fastq --run test_run_bcl/ --csv test_run_scripts/indices/test_run.csv &> mkfastq.log
echo "Running cellranger count"
echo ""
apptainer run -B /fast ../oscar-counting_latest.sif cellranger count --id test_run_sample --fastqs test_run_fastq/outs/fastq_path/H35KCBCXY/ --sample test_sample --localcores $num_cores --transcriptome $reference --chemistry SC3Pv3 --no-bam &> count.log
